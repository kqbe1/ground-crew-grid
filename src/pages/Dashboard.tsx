...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      // Fetch 6 months of tasks for the area chart
      ...
      const [
        ...
        supabase.from("maintenance_schedules").select("id", { count: "exact", head: true }).gte("next_due_date", monthStart).lte("next_due_date", monthEnd),
        supabase.from("maintenance_schedules").select("id", { count: "exact", head: true }).gte("next_due_date", nextMonthStart).lte("next_due_date", nextMonthEnd),
      ...
      // 1. Interventions by type (pie chart)
      ...
      setInterventionsByType(
        Object.entries(typeCounts)
          .map(([key, value]) => ({
            name: INTERVENTION_TYPE_LABELS[key] || key,
            value,
          }))
      ...
      setTaskStatusDistribution(
        Object.entries(statusCounts).map(([key, value]) => ({
          ...
          value,
          color: STATUS_COLORS[key] || "hsl(220, 10%, 46%)",
        }))
      );

      // 3. Monthly activity (area chart – last 6 months)
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const ficheCount = sheets.filter((s) => {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold">Statut des travaux</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-52">
