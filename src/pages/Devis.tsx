  ...
  const filtered = quotes.filter((q) => {
    ...
    if (search) {
      ...
      const match =
        ...
        q.client_city?.toLowerCase().includes(s) ||
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
              ...
              {isMobile ? (
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className={`${QUOTE_STATUS_COLORS[quote.status]} text-white text-xs`}>
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => handleDelete(e, quote.id)}>
                ...
                <>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {INSTALLATION_TYPE_LABELS[quote.installation_type]} · {quote.profiles?.full_name}
                    </div>
                  </div>
                  <div className="text-sm text-muted-foreground">{quote.client_city}</div>
                  <div className="text-xs text-muted-foreground whitespace-nowrap">
                    {format(new Date(quote.created_at), "d MMM yyyy HH:mm", { locale: fr })}
                  </div>
                  <Badge className={`${QUOTE_STATUS_COLORS[quote.status]} text-white text-xs`}>
                    {QUOTE_STATUS_LABELS[quote.status]}
                  </Badge>
                  <Button variant="ghost" size="icon" onClick={(e) => handleDelete(e, quote.id)}>
      ...
      <DevisDetailDialog
        ...
        onUpdated={() => { fetchQuotes(); setSelected(null); }}
      />
    </div>
  );
}
