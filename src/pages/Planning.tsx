...
function PlanningInner() {
  ...
  return (
    <div className="p-2 flex flex-col h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      {viewMode === "day" && (
        <div ref={dragScrollRef} className="border border-border rounded-xl overflow-x-auto overflow-y-hidden bg-card shadow-sm cursor-grab flex-1 min-h-0">
          <div className="grid" style={{ gridTemplateColumns: `80px repeat(${Math.max(displayedWorkers.length, 1)}, 180px)`, minWidth: `${80 + Math.max(displayedWorkers.length, 1) * 180}px` }}>
            ...
            {HOURS.map((hour) => (
              <div key={`row-${hour}`} className="contents">
                <div className="border-b border-border px-1 text-[10px] text-muted-foreground text-right pr-2 flex items-start justify-end pt-0.5 font-medium" style={{ height: `calc((100vh - 11.5rem) / ${HOURS.length})` }}>
                  ...
                  return (
                    <ContextMenu>
                      <ContextMenuTrigger asChild>
                        <div
                          key={`cell-${hour}-${w.id}`}
                          className="border-b border-l border-border relative" style={{ height: `calc((100vh - 11.5rem) / ${HOURS.length})` }}
                        >
