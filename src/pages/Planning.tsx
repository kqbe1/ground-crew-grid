          ...
          <div className="grid" style={{ gridTemplateColumns: `80px repeat(${Math.max(displayedWorkers.length, 1)}, 180px)`, minWidth: `${80 + Math.max(displayedWorkers.length, 1) * 180}px` }}>
            ...
            {HOURS.map((hour) => (
              <div key={`row-${hour}`} className="contents">
                <div className="border-b border-border px-1 text-[10px] text-muted-foreground text-right pr-2 flex items-start justify-end pt-0.5 font-medium" style={{ height: `calc((100vh - 7rem) / ${HOURS.length})` }}>
                  ...
                  return (
                    <ContextMenu>
                      <ContextMenuTrigger asChild>
                        <div
                          key={`cell-${hour}-${w.id}`}
                          className="border-b border-l border-border relative" style={{ height: `calc((100vh - 7rem) / ${HOURS.length})` }}
                        >
